...
const AnimeFavorito = require('../models/animeFavorito');
...
router.get('/favoritos', autenticar, async (req, res) => {
  try {
    const favoritos = await AnimeFavorito.findAll({
...
router.post('/favoritos', autenticar, async (req, res) => {
  ...
  try {
    const jaExiste = await AnimeFavorito.findOne({
    ...
    const favorito = await AnimeFavorito.create({
      animeId,
      titulo,
      dados,
      userId: req.user.id,
...
router.delete('/favoritos/:id', autenticar, async (req, res) => {
  try {
    const deleted = await AnimeFavorito.destroy({
      where: {
        userId: req.user.id,
